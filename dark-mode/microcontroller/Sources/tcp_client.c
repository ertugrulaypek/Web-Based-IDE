...
static err_t connection_callback(void *arg, struct tcp_pcb *tpcb, err_t err) {
	...
	const char* message = "HEAD /dark-mode/toggle HTTP/1.0\r\n\r\n";
	uint16_t messageLen = strlen(message);

	err_t result = tcp_write(tpcb, message, messageLen, TCP_WRITE_FLAG_COPY);
...
static void create_tcp_connection() {
	...
	ipaddr_aton("35.234.93.221", &remoteIp);
	ip_addr_set(&localIp, &netif.ip_addr);

	pcb = tcp_new();
	tcp_bind(pcb, &localIp, local_port);

	tcp_connect(pcb, &remoteIp, remote_port, connection_callback);
